...
export const Squares: FC<SquaresProps> = ({
  direction = 'right',
  speed = 1,
  borderColor = '#999',
  squareSize = 40,
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      hoveredSquareRef.current = {
        x: Math.floor(
          (e.clientX - rect.left + (gridOffset.current.x % squareSize)) / squareSize
        ),
        y: Math.floor(
          (e.clientY - rect.top + (gridOffset.current.y % squareSize)) / squareSize
        ),
      };
    };
